...
export function DataProvider({ children }) {
  ...
  const toggleDone = async (dateKey, lineIdx) => {
    ...
    const cur = list[lineIdx];
    if (!cur) return;
    const updated = await adapter.updateTask(cur.id, {
      is_done: !cur.is_done,
      done: !cur.is_done,
    });
    const next = list.slice();
  ...
  const moveTaskToDate = async (dateKey, lineIdx, targetKey) => {
    ...
    const cur = list[lineIdx];
    if (!cur) return;
    const updated = await adapter.updateTask(cur.id, { due_date: targetKey });
  ...
  const duplicateTask = async (dateKey, lineIdx) => {
    ...
    const next = list.slice();
    next.splice(lineIdx + 1, 0, created);
    setDay(dateKey, next);
  };

  const value = useMemo(
    () => ({
      // ✅ giữ key cũ và thêm alias "tasks" cho App.jsx
      tasksByDate,
      tasks: tasksByDate,
      getTasksForDate,
      getTask,               // ✅ expose getTask
      loadRange,
      updateMeta,
      removeTask,
      toggleDone,
