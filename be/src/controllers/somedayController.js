...
exports.createColumn = async (req, res, next) => {
  try {
    ...
    const { rows } = await db.query(
      'INSERT INTO someday_columns (calendar_id, title, display_order) VALUES ($1,$2,$3) RETURNING *',
      [calId, title, Number(m[0].next) || 0]
    );
...
exports.createSomedayTask = async (req, res, next) => {
  try {
    ...
    const {
      ...
      subtasks = [], attachments = [], repeat_info = { type: 'never' }, reminder_info = null, links = []
    } = req.body;
...
exports.updateSomedayTask = async (req, res, next) => {
  try {
    const id = Number(req.params.id);
    const calId = req.calendarId;
    const { text, notes, is_done, color, subtasks, attachments, repeat_info, reminder_info, links } = req.body;
